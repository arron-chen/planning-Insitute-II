<template>
  <div id="entiretyTable" class="entirety-table">
    <div class="search-item" style="border: 1px solid #dbe3e7;border-radius: 4px">
      <el-form ref="form" :model="form" label-width="80px">
        <el-form-item label="年份" style="float:left;margin-bottom:8px;">
          <el-select v-model="form.year" placeholder="请选择年度" style="width:150px;" size="small">
        ...
        <el-form-item label="数据区划" style="float:left;margin-bottom:8px;">
          <el-select v-model="form.xzdm" placeholder="请选择数据区划" style="width:150px;" size="small">
        ...
        <el-form-item label="汇总方案" style="float:left;margin-bottom:8px;margin-left:20px;">
          <el-select v-model="form.tname" placeholder="请选择汇总方案" style="width:150px;" size="small">
        ...
        <el-form-item label="批次" style="float:left;margin-bottom:8px;">
          <el-select v-model="form.sjpc" placeholder="请选择批次" style="width:150px;" size="small">
    ...
    <div id="entiretyBox" class="panel-content">
      <div data-options="region:'west',split:true,tools:'#entiretyTool'" title="列表" style="width:250px;">
        <el-tree :data="treeData" :props="defaultProps" @node-click="treeNodeClick" v-loading.body="loadTree" :render-content="renderContent"></el-tree>
      </div>
       <div id="entiretyTool">
      ...
      <div data-options="region:'center',split:true">
        <div class="tool-box" v-if="toolDisplay">
          ...
          <el-button type="primary" size="small" @click="exportExcelMore" :loading="exportExcelMoreLoading">批量导出</el-button>
          <el-button type="primary" size="small" @click="exportTemplate">模板导出</el-button>
          <el-button type="primary" size="small" @click="templateManager">模板管理</el-button>
        ...
        <div class="table-item" v-if="isTable">
          <el-table :data="tableData.Rows" border v-loading.body="listLoading" :max-height="tableHeight"  >
    ...
    <el-dialog title="修改节点" :visible.sync="dialogFormVisible4" size=tiny >
      ...
      <el-input size="small" placeholder="请输入节点名称" v-model="trendTreeLabelName" v-else></el-input>
      <div slot="footer" class="dialog-footer">
    ...
    <el-dialog title="模板管理" :visible.sync="dialogFormVisible9">
        <el-table :data="templateData"  border style="width: 100%" >
            <el-table-column prop=" " label="启用模板" width="80"  align="center">
                <template scope="scope">
                    <el-radio class="radio" v-model="templateRid" :label="scope.row.rid"  @change.native="raidoChange(scope.$index,scope.row)">&nbsp;</el-radio>
                </template>
             </el-table-column>
            <el-table-column   prop="exlName" label="模板名称">   </el-table-column>
             <el-table-column   label="操作" width="100"  align="center">
                <template slot-scope="scope" v-if="scope.row.rid !='disable'">
                  <el-button size="small" @click.native="deltemplate(scope.$index, scope.row)">删除</el-button>
                </template>
             </el-table-column>
       </el-table>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button  @click="dialogFormVisible9 = false" size="small">取 消</el-button>
        <el-button type="primary" @click="useTemplate" size="small">确 定</el-button>
      </div>
    </el-dialog>
  ...
  import TreeItem from './treeItem.vue'
   ...
   export default {
    ...
    data(){
      return {
       ...
       requertHeader: {
        ...
        TreeLabelName2 : '',	//发布报表树增加子节点的
        TreeLabelName3 :'',//发布报表树增加根节点的
        TreeLabelDescribe : '', //发布报表树子节点描述
        TreeLabelDescribe2: '', //发布报表树根节点描述
        isTable : true,
        tableHtml : '',
        trendTree : [], //发布趋势树
        trendTreeId : '', //发布趋势树选中ID
        trendTreeLabelName : '', //发布趋势树修改的
        trendTreeLabelName2 : '',  //发布趋势树增加子节点的
        trendTreeLabelName3 : '',  //发布趋势树增加子节点的
        trendTreeLabelDescribe : '', //发布趋势树子节点描述
        trendTreeLabelDescribe2 : '', //发布趋势树根节点描述
        flag: '',
        bReport:'', //是否已发布报表
        bTrendChart:'',//是否已发布趋势
        tableYaer:'',//表年份
    ...
    mounted(){
      ...
      this.$nextTick(() =>{
        this.tableHeight = $('#entiretyBox').height() - 80
        $('#entiretyBox').layout()
      })
      //加载左侧目录树
      this.loadLeftTree();
    ...
    methods : {
      ...
      getTemplateData(){
        ...
        api.fetchShowTemplateInfo({params:data})
          .then(res=>{
              console.log(res.data)
              this.templateData = res.data
              for(var i=0;i<this.templateData.length;i++){
                  if(this.templateData[i].status == 1){
                    this.templateRid =  this.templateData[i].rid
                      console.log("display: "+this.templateRid)
                    this.oldTemplateRid =  this.templateRid
                      console.log("old: "+this.oldTemplateRid)
                }
              }
          })
      ...
      deltemplate(index, row){
        ...
        this.$confirm('是否删除模板, 是否继续?', '提示', {
          ...
          api.fetchDeleteTemplate({params:roleId})
            .then(res=>{
              console.log(res.data);
              this.getTemplateData()
            })
      ...
      //模板选择改变时
      raidoChange(a,row){
        this.newTemplateRid = row.rid;
        console.log("old: "+this.oldTemplateRid)
        console.log("display: "+this.templateRid)
        console.log("selcet: "+row.rid)
      },
      //确定启用模板
      useTemplate(){
          let data = {
             rid:this.newTemplateRid,
             oldRid:this.oldTemplateRid
          }
        console.log(data)
        api.fetchEnableTemplate({params:data})
           .then(res=>{
              console.log(res);
          ...
          api.fetchGetXZDMs({params : datas})
            .then(res =>{
              console.log(res.data)
      ...
      loadLeftTree(){
        ...
        api.fetchSearchProTable({params : cata})
          .then(res =>{
            this.loadTree = false
            console.log(res.data)
      ...
      //发布报表树点击选中
      FormTreeNodeClick : function(eve, node){
        console.log('发布树点击选中')
      ...
      submitDel : function(){
        if(this.flag == true){
          ...
          console.log('this.TreeRidNum:'+this.TreeRidNum)
      ...
      printTable : function(){
          ...
          let codestr = document.getElementById("tableItem").innerHTML;   //获取需要生成pdf页面的div代码
      ...
      exportExcelMore:function(){
        ...
        if(this.showTableRid == ''){
            ...
            let exportTable = {
              ...
              this.exportExcelMoreLoading = true
              api.fetchBatchExportExcel({params : exportTable})
                ...
                if(res.data.message){
                 this.$message({
                  ...
                  console.log(res.data)
                  this.exportExcelMoreLoading = false
                  window.location.href = this.serviceUrl+"/mainWeb" + res.data + ""
                }
              })
          }
